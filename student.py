...
def getVolBook(stuId):
    ...
    database.execute("SELECT volId FROM stu_vol WHERE stuId=%s"%(stuId))
    r = database.fetchall()
    respdata['type'] = "SUCCESS"
    respdata['message'] = "获取成功"
    respdata['rec'] = []
    for i in r:
        database.execute("SELECT volName, volTimeInside, volTimeOutside, volTimeLarge, status FROM stu_vol WHERE vid=%s"%(i[0]))
